...
export class ProductListComponent implements OnInit{
  ...
  handleListProduct(){
    ...
    //check we have different category id than previous
    //Note: angular will reuse a component if it is currently being viewed
    //
    //if we have different category id than previous
    //then set the pageNumber back to 1
    if(this.preveousCategoryId != this.currentCategoryId){
      this.thePageNumber = 1;
    }
    this.preveousCategoryId = this.currentCategoryId;

    console.log(`currentCategoryId=${this.currentCategoryId}, thePageNumber=${this.thePageNumber}`);


    //now get the product of given categotyId
